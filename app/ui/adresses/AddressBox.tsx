...
export default function AddressBox() {
    ...
    //-------------------//BEGIN DELETE ADDRESS//--------------------//
      ...
      const updateAddress = async(id: string, values: IAddress) => {
        try {
          const response = await axiosClient.put(`${env.API_URL}/address/${id}`,
            values
          );
          if(response.status === 200) {
          ...
          console.error('update address is failed');
        }
      }

      //gọi hàm update address
      const handleUpdate = async (id: string, values: IAddress) => {
